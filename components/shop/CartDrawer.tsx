    ...
    <>
      ...
      >
        <header className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          ...
          <button onClick={() => setOpen(false)} className="p-1 text-gray-400 hover:text-[#DED6C3]" aria-label="Close cart">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <div className="inline-flex items-center bg-white/[0.06] border border-white/15 rounded-sm text-sm text-gray-200">
                      <button onClick={() => setQty(item.productId, item.qty - 1)} className="px-2 py-1 text-gray-400 hover:text-[#DED6C3]" aria-label="Decrease">−</button>
                      <span className="min-w-[1.5rem] text-center text-xs font-semibold">{item.qty}</span>
                      <button onClick={() => setQty(item.productId, item.qty + 1)} className="px-2 py-1 text-gray-400 hover:text-[#DED6C3]" aria-label="Increase">+</button>
                    </div>
