...
export default function GlassImage({
  ...
  return (
    ...
    <div className={`absolute inset-0 overflow-hidden bg-[#1f4f57] ${className}`}>
      {/* The photo */}
      {plain ? (
        <img
          src={src}
      ...
      <div
        className={`absolute inset-0 bg-[#1f4f57] transition-opacity duration-500 ${s.film}`}
        aria-hidden
      />

      {/* Surface sheen — the light catching the top of the glass */}
      <div
        className="pointer-events-none absolute inset-0 bg-gradient-to-b from-white/[0.12] via-transparent to-[#1f4f57]/20"
        aria-hidden
      />
