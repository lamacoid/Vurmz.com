      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {files.map(f => {
            ...
            return (
              <div key={f.id} className="flex items-center gap-3 bg-[#235158] border border-white/5 rounded-lg px-3 py-2.5">
                <button
                  onClick={() => previewable ? setPreview(f) : window.open(f.url, '_blank')}
                  className="flex-shrink-0 w-14 h-14 bg-[#1c474e] rounded-md overflow-hidden flex items-center justify-center group relative"
                ...
                >
                  {isImage(f.mimeType) ? (
                    ...
                    <span className="text-[10px] font-mono font-bold text-[#6BB8B2] tracking-wider">PDF</span>
                  ) : (
