    ...
    <div className="max-w-3xl mx-auto p-6 sm:p-10">
      ...
      {loading ? (
        ...
        <div
          ...
          className="border-2 border-dashed border-white/10 rounded-xl p-14 text-center cursor-pointer hover:border-[#2FE6C4]/40 transition-colors"
        >
              ...
              <div key={f.id} className="flex items-center gap-3 bg-[#1A4F48] border border-white/5 rounded-lg px-3 py-2.5">
                ...
                >
                  {isImage(f.mimeType) ? (
                    ...
                    <span className="text-[10px] font-mono font-bold text-[#2FE6C4] tracking-wider">PDF</span>
                  ) : (
                ...
                <button onClick={() => previewable ? setPreview(f) : window.open(f.url, '_blank')} className="flex-1 min-w-0 text-left">
                  <p className="text-sm text-cream hover:text-[#2FE6C4] truncate">{f.filename}</p>
