...
export default function AccountLogin() {
  ...
  return (
    <div className="max-w-md mx-auto p-6 sm:p-10">
      ...
      {status === 'sent' ? (
        <div className="bg-[#1A4F48] border border-[#6FB6AC]/20 rounded-xl p-6">
          ...
          <p className="text-xs text-gray-500 mt-4">Didn&rsquo;t get it? Check spam, or <button onClick={() => setStatus('idle')} className="text-[#6FB6AC] hover:underline">try again</button>.</p>
        </div>
      ) : (
        <form onSubmit={submit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-[#1A4F48] border border-white/5 rounded-md px-3 py-2.5 text-sm text-cream outline-none focus:border-[#6FB6AC]"
            />
          </div>
