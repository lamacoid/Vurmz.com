...
export async function GET(req: NextRequest) {
  try {
    ...
    const { results } = await db.prepare(
      "SELECT id, r2_key, filename, mime_type, size_bytes, uploaded_at FROM customer_files WHERE customer_id = ? AND kind = 'file' AND deleted_at IS NULL ORDER BY uploaded_at DESC"
