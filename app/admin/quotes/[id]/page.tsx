...
export default function QuoteDetailPage() {
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json() as Quote & { message?: string; invoicePaid?: boolean }
        setQuote(data)
                ...
                {quote.status === 'pending-approval' && (
                  <>
                    <div className="bg-orange-50 border border-orange-200 p-3 rounded mb-3">
                      <p className="text-sm font-medium text-orange-800">Ready to Start!</p>
                      <p className="text-xs text-orange-700 mt-1">
                        Customer placed an order for ${quote.price?.toFixed(2)}. Start the order to add it to your work queue.
                      </p>
                    </div>
                    <button
