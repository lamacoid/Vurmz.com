      ...
      <div className="bg-[#1A4F48] border border-white/5 rounded-xl p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input type="date" value={dueDate} onChange={e => setDueDate(e.target.value)} className="w-full bg-[#143E38] border border-white/5 rounded-md px-3 py-2 text-sm text-cream outline-none focus:border-[#6FB6AC]" />
          </div>
        </div>
        <div>
          <label className="text-[11px] uppercase tracking-wider text-gray-500 block mb-1 font-semibold">Notes</label>
          <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={4} className="w-full bg-[#143E38] border border-white/5 rounded-md px-3 py-2 text-sm text-cream outline-none focus:border-[#6FB6AC]" placeholder="Details, specs, production notes…" />
        </div>
