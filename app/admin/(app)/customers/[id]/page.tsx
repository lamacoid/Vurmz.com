...
function InlineInput({ value, onSave, placeholder, className }: { value: string; onSave: (v: string) => void; placeholder?: string; className?: string }) {
  ...
  return (
    <input
      ...
      className={`w-full bg-transparent text-cream outline-none border-b border-transparent focus:border-[#6BB8B2]/40 ${className ?? 'text-sm'}`}
    />
  )
}
...
function TagEditor({ value, onSave }: { value: string[]; onSave: (v: string[]) => void }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      {value.map(t => (
        <span key={t} className="bg-[#6BB8B2]/10 border border-[#6BB8B2]/20 text-[#6BB8B2] text-[11px] px-2 py-0.5 rounded-full inline-flex items-center gap-1">
          {t}
