...
const PROOF_CHIP: Record<string, { label: string; cls: string }> = {
  ...
  approved: { label: 'Proof ✓', cls: 'bg-[#6BB8B2]/15 text-[#6BB8B2] border-[#6BB8B2]/30' },
}
...
function StatCard({ title, value, sub, href, icon, alert }: {
  ...
  return (
    <Link href={href} className={`group bg-[#235158] rounded-xl p-5 border transition-colors ${alert ? 'border-amber-500/40 hover:border-amber-400/60' : 'border-white/5 hover:border-[#6BB8B2]/30'}`}>
      <div className="flex items-start justify-between mb-3">
        <p className="text-[11px] text-gray-400 uppercase tracking-wider">{title}</p>
        <Icon name={icon} className={`w-4 h-4 opacity-70 ${alert ? 'text-amber-300' : 'text-[#6BB8B2]'}`} />
      </div>
        ...
        <>
          ...
          <section className="mb-8">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/admin/orders" className="text-xs text-[#6BB8B2] hover:text-cream">Order board →</Link>
            </div>
            {dash.orders.length === 0 ? (
              <div className="bg-[#235158] border border-white/5 rounded-xl p-8 text-center text-gray-400 text-sm">
                No open orders. Go send five outreach texts.
              </div>
            ) : (
              <div className="bg-[#235158] border border-white/5 rounded-xl divide-y divide-white/5">
                {dash.orders.map(o => (
