...
const STATUS_COLORS: Record<string, string> = {
  ...
  delivered: 'bg-[#2FE6C4]/20 text-[#2FE6C4]',
...
function NewJobForm({ onSave, onCancel }: { onSave: (data: any) => void; onCancel: () => void }) {
  ...
  return (
    <div className="px-4 py-6 max-w-lg mx-auto">
      <button onClick={onCancel} className="text-xs text-[#2FE6C4] mb-4 flex items-center gap-1">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3 h-3"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" /></svg>
        Cancel
      </button>
      ...
      <div className="space-y-3">
        <input
          ...
          className="w-full px-3 py-2.5 bg-[#1A4F48] border border-[#2d4a47] rounded-lg text-sm text-cream placeholder:text-gray-500 focus:outline-none focus:border-[#2FE6C4]/50"
        />
        <select
          value={form.type}
          onChange={e => setForm(f => ({ ...f, type: e.target.value }))}
          className="w-full px-3 py-2.5 bg-[#1A4F48] border border-[#2d4a47] rounded-lg text-sm text-cream focus:outline-none focus:border-[#2FE6C4]/50"
        >
        ...
        <textarea
          ...
          rows={3}
          className="w-full px-3 py-2.5 bg-[#1A4F48] border border-[#2d4a47] rounded-lg text-sm text-cream placeholder:text-gray-500 focus:outline-none focus:border-[#2FE6C4]/50 resize-none"
        />
        <textarea
          placeholder="Notes"
          value={form.notes}
