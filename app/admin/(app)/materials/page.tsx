      ...
      {loading ? (
        ...
        <div className="space-y-5">
          {Object.entries(byCategory).map(([cat, list]) => (
            <div key={cat} className="bg-[#235158] border border-white/5 rounded-xl overflow-hidden">
...
function Row({ material, onPatch, onRemove }: { material: Material; onPatch: (p: Partial<Material>) => void; onRemove: () => void }) {
  ...
  return (
    <div>
      ...
      {expanded && (
        <div className="px-4 pb-3 space-y-2 bg-[#1c474e]/50">
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full bg-[#1c474e] border border-white/5 rounded px-2 py-1.5 text-xs text-cream outline-none focus:border-[#6BB8B2]"
            />
          </div>
        </div>
      )}
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="w-full bg-[#235158] border border-white/5 rounded px-2 py-1 text-xs text-cream outline-none focus:border-[#6BB8B2]"
      />
    </label>
  )
}
