      ...
      {loading ? (
        ...
        <div className="space-y-5">
          {Object.entries(byCategory).map(([cat, list]) => (
            <div key={cat} className="bg-[#243B39] border border-white/5 rounded-xl overflow-hidden">
  ...
  return (
    <div>
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <button onClick={() => setExpanded(!expanded)} className="text-xs text-[#6BB8B2] hover:underline">{expanded ? 'Hide' : 'Details'}</button>
        <button onClick={onRemove} className="text-xs text-red-300 hover:text-red-200">×</button>
      </div>
      {expanded && (
        <div className="px-4 pb-3 space-y-2 bg-[#1a2f2e]/50">
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full bg-[#1a2f2e] border border-white/5 rounded px-2 py-1.5 text-xs text-cream outline-none focus:border-[#6BB8B2]"
            />
          </div>
        </div>
      )}
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="w-full bg-[#243B39] border border-white/5 rounded px-2 py-1 text-xs text-cream outline-none focus:border-[#6BB8B2]"
      />
    </label>
  )
}
